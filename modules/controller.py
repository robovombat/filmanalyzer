...
class TuiController:
    ...
    def _execute_command(self) -> str:
        """🧠 Parse, sanitize, execute user command string"""

        # 🔹 Collect + sanitize typed input
        cmd_str = ''.join(self.current_input).strip()

        # 🔸 If prompt-like ">" is typed (e.g. > extract), strip it
        ...
        # 🔹 Tokenize base command and arguments
        parts = cmd_str.split()
        ...
        try:
            # 🔹 Run command from the registry
            result = self.command_registry[base_cmd](*args)
            response = f"[OK] {result}" if result else "[OK] Done"

            # 🔍 Post-process extract frame count (if matched)
            if base_cmd == "extract" and "Extracted" in result:
                try:
                    count = int(result.split("Extracted")[1].split("frames")[0].strip())
                    response += f"\n[info] ✅ Frame extract complete: {count} frames"
                except Exception:
                    response += "\n[warn] Could not parse extract summary"
